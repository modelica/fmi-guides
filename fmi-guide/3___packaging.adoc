...
In the initial 3.0 release, this specification was ambiguous and broken: While the intent always was for all functions to be present, it could be read as only requiring the functions of those interface types the FMU supports having to be present.
...
The situation for optional features is unclear.
...
Similarly it is recommended for exporters always generate all functions specified in the relevant standards.
